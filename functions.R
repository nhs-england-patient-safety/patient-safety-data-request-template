...
replace_filter <- function(string_formatted, i, dataset) {
  ...
  if (filter_category == "in" | filter_category == "not_in") {
    if (filter_category == "in") {
      ...
      # convert to a string (to make consistent with next section) - there are already spaces here so use "," not ", "
      value_string <- str_c(value_string, collapse = ",")
      not_string <- ""
    } else{
      #the first element is the ! so the second element has the rest of the query
    ...
    value_vec_old <- as.character(value_vec_old) #make sure each element is a character, not string
    #find column name
    ...
    #column name is second element of i 
    column <- as.character(i[2])
    # convert column code to column name
    column_new <- get_column_text(column, dataset)
    # value is third item of i
    value_old <- i[[3]]
    # operator is first item of i
    operator <- as.character(i[1])
    #convert value to full version
    ...
    # this has an awkward structure " " + A001 + " " - so pull out just the column name
    column <- as.character(i[2]) %>%
      str_extract("[a-zA-Z0-9_.-]+")
    # convert column name to full version
    column_new <- get_column_text(column, dataset)
    
    #value is third element
    value_old <- i[[3]]
    # operator is first element
    operator <- as.character(i[1])
    
    #% symbol is required in search - remove it to make output more clear and allow conversion of value 
    ...
    #work out if this is an !is.na or is.na- flag if it is not na
    not_na <- as.character(i)[1] == "!"
    # pull out column code
    column <- as.character(i)[2] %>%
      str_extract_all("[a-zA-Z0-9_-]{3,}")
    #convert column name
    column_new <- get_column_text(column , dataset)
    #build up new version of filter, depending on whether the search is !is.na or is.na
    if (not_na) {
  ...
  # replace filter initial with filter_nice. if no filter category was found, the replace will not run
...
#' 
#' Finds the category of a filter - can be equals, in , not in, filter_na, multi or NA if not possible to categorise
#'
#' @param i filter to find category of (call)
#'
#' @return string representing category (or NA)
find_filter_category <- function(i) {
  
  # look for specific patterns found in the different categories
  ...
  #work out if criteria for any category is met
