...
if(nrow(nrls_filtered_text) != 0){
  ...
  nrls_age_categorised <- nrls_labelled %>%
    # this is required because are concatenating the columns
    ...
    mutate(concat_col = paste(IN07, IN10, IN11, IN05_TEXT, PD05_LVL1, PD05_LVL2, PD05_TEXT, sep = " "),
  ...
  if (nrow(nrls_neopaed) != 0) {
    ...
    print(glue("**The neopaeds search has produced no results in {dataset}**"))  }
} else {
  print(glue("**The categorical and text search has produced no results in {dataset}**"))
