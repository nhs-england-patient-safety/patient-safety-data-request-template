...
if (check_and_log_empty_result(lfpse_filtered_text, dataset, "text")) {
  ...
  lfpse_age_classified <- lfpse_filtered_text |>
    mutate(
      concat_col = paste(F001, AC001, OT003, A008_Other, A008, A002, DMD002, DMD004, sep = " "),
      P004_days_validated = ifelse(between(P004_days, 0, 365.25 * 120), P004_days, NA),
      age_category = case_when(
        between(P004_days_validated, 0, 28) ~ 'neonate',
        between(P004_days_validated, 29, 365.25 * 18) ~ 'paediatric',
        P004_days_validated >= 365.25 * 18 ~ 'adult',
