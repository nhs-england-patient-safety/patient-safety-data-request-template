...
# sql_date_filter uses original column names for the DB query
sql_date_filter <- if (date_type == 'occurring') expr(OccurredDate) else expr(as.Date(OriginalSubmissionDate))

# date_filter uses renamed column names for post-collect
date_filter <- if (date_type == 'occurring') expr(occurred_date) else expr(as.Date(reported_date))
...
valid_refs_tbl <- copy_to(con_lfpse, valid_refs, name = "#valid_refs",
                          temporary = TRUE, overwrite = TRUE)

lfpse_filtered_categorical <-
  lfpse_analysis_table |>
  inner_join(valid_refs_tbl, by = "Reference") |>
...
toc_lfpse <- Sys.time()
log_extraction_time(dataset, toc_lfpse - tic_lfpse)
...
for (col in lfpse_text_columns) {
  ...
  lfpse_filtered_categorical[[col]] <- if_else(
    ...
    lfpse_filtered_categorical[[fill_col]],
  ...
  lfpse_age_classified <- lfpse_age_validated |>
    mutate(
      ...
      age_category = case_when(
        ...
        is.na(P004_days_validated) ~ 'unknown',
        .default = 'other'
      ),
