...
if (nrls_categorical == 0) {
  nrls_categorical <- dplyr::expr(1 == 1)
}

# read tables ####

nrls <- dplyr::tbl(con_nrls, dbplyr::in_schema("art", "vw_clean"))

organisations <- dplyr::tbl(con_nrls, dbplyr::in_schema("art", "trust_details")) |>
  dplyr::collect()
...
nrls_filtered_categorical <- nrls_parsed |>
  ...
  dplyr::select(dplyr::any_of(unname(rename_lookup[["NRLS"]])))|>
  # collecting here so that we can apply text filters later
  dplyr::collect() |>
  dplyr::mutate(year_reported_or_occurred = lubridate::year(!!date_filter),
...
if (sum(!is.na(text_terms)) > 0) {
  ...
  nrls_filtered_text_precursor <- nrls_filtered_categorical |>
    dplyr::mutate(concat_col = paste(IN07, IN03_TEXT, IN05_TEXT, IN11, IN10, MD05, MD06, MD30, MD31, DE01_TEXT, DE03, sep = " "))
  ...
  message(glue::glue("{dataset} text search retrieved {format(nrow(nrls_filtered_text), big.mark = ',')} incidents."))
} else {
...
if (nrow(nrls_filtered_text) != 0) {
  ...
  tidyr::pivot_longer(cols = dplyr::any_of(codes$col_name)) |>
    dplyr::left_join(codes, by = c(
      "name" = "col_name",
      "value" = "SASCODE"
    )) |>
    dplyr::select(!value) |>
    ...
    dplyr::left_join(organisations, by = c("RP07" = "ORGANISATIONCODE"))
  ...
  nrls_age_categorised <- nrls_labelled |>
    dplyr::mutate(
      ...
      age_less_28_days= dplyr::between(AGE_AT_INCIDENT, 0, 28/365),
      ...
      neonate_terms_text = stringr::str_detect(concat_col, neonatal_terms),
      ...
      paed_terms_text = stringr::str_detect(concat_col, paediatric_terms))|>
  ...
  print(stringr::str_glue("nrls_neopaed has {nrow(nrls_neopaed)} incidents"))
  ...
  if (nrow(nrls_neopaed) != 0) {
    ...
    if (sampling_strategy == "default") {
      if (nrow(nrls_neopaed) > 300) {
        ...
        nrls_death_severe <- nrls_neopaed |>
          dplyr::filter(PD09  %in% c("Death", "Severe"))
        
        set.seed(123)
        
        nrls_moderate <- nrls_neopaed |>
          dplyr::filter(PD09 == "Moderate") |>
        ...
        nrls_low_no_other <- nrls_neopaed |>
          dplyr::filter(!PD09 %in% c("Moderate", "Severe", "Death")) |>
          dplyr::sample_n(min(dplyr::n(), 100))
        
        nrls_sampled <- dplyr::bind_rows(
          nrls_death_severe,
          nrls_moderate,
          nrls_low_no_other
        )
    ...
    nrls_for_summary_table_unsampled <- nrls_neopaed  |>
      dplyr::select(dplyr::any_of(rename_lookup[["NRLS"]]), dplyr::starts_with("group_"))
    ...
    message(glue::glue("**The search strategy has produced no results in {dataset}**"))
    message(glue::glue("Moving on..."))
  }
